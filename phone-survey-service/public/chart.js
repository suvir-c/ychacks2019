...
var indexOf = helpers.indexOf = function(arrayToSearch, item){
...
var where = helpers.where = function(collection, filterCallback){
            var filtered = [];
...
var findNextWhere = helpers.findNextWhere = function(arrayToSearch, filterCallback, startIndex){
...
var findPreviousWhere = helpers.findPreviousWhere = function(arrayToSearch, filterCallback, startIndex){
...
var isNumber = helpers.isNumber = function(n){
            return !isNaN(parseFloat(n)) && isFinite(n);
        };

        
var max = helpers.max = function(array){
            return Math.max.apply( Math, array );
        };

        
var min = helpers.min = function(array){
            return Math.min.apply( Math, array );
        };

        
var cap = helpers.cap = function(valueToCap,maxValue,minValue){
...
var getDecimalPlaces = helpers.getDecimalPlaces = function(num){
...
var toRadians = helpers.radians = function(degrees){
            return degrees * (Math.PI/180);
        };

        // Gets the angle from vertical upright to the point about a centre.
        
var getAngleFromPoint = helpers.getAngleFromPoint = function(centrePoint, anglePoint){
...
var aliasPixel = helpers.aliasPixel = function(pixelWidth){
            return (pixelWidth % 2 === 0) ? 0 : 0.5;
        };

        
var splineCurve = helpers.splineCurve = function(FirstPoint,MiddlePoint,AfterPoint,t){
...
var generateLabels = helpers.generateLabels = function(templateString,numberOfSteps,graphMin,stepValue){
...
var animationLoop = helpers.animationLoop = function(callback,totalSteps,easingString,onProgress,onComplete,chartInstance){
...
var getRelativePosition = helpers.getRelativePosition = function(evt){
            var mouseX, mouseY;
...
var bindEvents = helpers.bindEvents = function(chartInstance, arrayOfEvents, handler){
...
var unbindEvents = helpers.unbindEvents = function (chartInstance, arrayOfEvents) {
...
var getMaximumWidth = helpers.getMaximumWidth = function(domNode){
...
var getMaximumHeight = helpers.getMaximumHeight = function(domNode){
            var container = domNode.parentNode;
            // TODO = check cross browser stuff with this.
...
var getMaximumSize = helpers.getMaximumSize = helpers.getMaximumWidth;
 // legacy support
        
var retinaScale = helpers.retinaScale = function(chart){
            var ctx = chart.ctx,
...
var clear = helpers.clear = function(chart){
...
var longestText = helpers.longestText = function(ctx,font,arrayOfStrings){
            ctx.font = font;
            var longest = 0;
...
var newHeight = this.options.maintainAspectRatio ? newWidth / this.chart.aspectRatio : getMaximumHeight(this.chart.canvas);
  ...
  Chart.Type.extend = function(extensions){
    ...
    if (extensions.name || parent.prototype.name){
      let chartName = extensions.name || parent.prototype.name;
  ...
  Chart.Point = Chart.Element.extend({
    ...
    inRange: function(chartX,chartY){
      let hitDetectionRange = this.hitDetectionRadius + this.radius;
  ...
  Chart.Arc = Chart.Element.extend({
    inRange : function(chartX,chartY){
      let pointRelativePosition = helpers.getAngleFromPoint(this, {
        x: chartX,
        y: chartY,
      });
...
var leftX = this.x - halfWidth;

                
var rightX = this.x + halfWidth;

                
var top = this.base - (this.base - this.y);
...
var longestTextWidth = max([labelWidth,titleWidth]);
  ...
  Chart.Scale = Chart.Element.extend({
    ...
    buildYLabels : function(){
      ...
      let stepDecimalPlaces = getDecimalPlaces(this.stepValue);
...
var valueWidth = innerWidth/(this.valuesCount - ((this.offsetGridLines) ? 0 : 1));
...
var drawHorizontalLine = this.showHorizontalLines;
...
var drawVerticalLine = this.showVerticalLines;
...
var furthestRightIndex;

                
var furthestRightAngle;

                
var furthestLeft = 0;

                
var furthestLeftIndex;

                
var furthestLeftAngle;

                
var xProtrusionLeft;

                
var xProtrusionRight;
...
var maxWidthRadius;
    ...
    setCenterPoint: function(leftMovement, rightMovement){
      let maxRight = this.width - rightMovement - this.drawingArea;
...
var pointPosition;
        ...
        if (!this.lineArc){
          ...
          for (let i = this.valuesCount - 1; i >= 0; i--) {
...
var halfLabelsCount = this.labels.length/2;
...
var datasetIterator = function(dataset){
...
var nextPoint = function(point, collection, index){
                return helpers.findNextWhere(collection, hasValue, index) || point;
            };

            
var previousPoint = function(point, collection, index){
...
var helpers = Chart.helpers;
  ...
  Chart.Type.extend({
    ...
    getSegmentsAtEvent : function(e){
      ...
      let location = helpers.getRelativePosition(e);
...
var helpers = Chart.helpers;
  ...
  Chart.Type.extend({
    ...
    getPointsAtEvent : function(evt){
      let mousePosition = helpers.getRelativePosition(evt);

                
var fromCenter = helpers.getAngleFromPoint({
...
var pointIndex = Math.round((fromCenter.angle - Math.PI * 1.5) / anglePerIndex);
...
var ctx = this.chart.ctx;
      ...
      helpers.each(this.datasets,function(dataset){
        ...
        ctx.strokeStyle = dataset.strokeColor;
          ctx.beginPath();
        helpers.each(dataset.points,(point,index) => {
                    if (index === 0){
                        ctx.moveTo(point.x,point.y);
                    }
                    else{
